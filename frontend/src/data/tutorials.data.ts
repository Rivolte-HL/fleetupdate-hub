...
'`,
  ...
  TRUENAS: {
    id: 'TRUENAS',
    name: 'TrueNAS SCALE & Applications',
    icon: HardDrive,
    color: 'text-sky-400',
    badge: 'REST API v2.0',
    summary:
      'Supervision et mise à niveau centralisée du système TrueNAS (SCALE / CORE), de ses applications (Docker Compose sur SCALE 24.10+ et Charts Helm), avec snapshots ZFS automatisés avant mise à jour.',
    steps: [
      {
        title: '1. Générer une Clé API sur TrueNAS WebGUI',
        description:
          'Connectez-vous à votre interface TrueNAS SCALE en tant qu’administrateur ou root.\nEn haut à droite, cliquez sur votre profil utilisateur ou sur la roue crantée (Paramètres) > API Keys > Add.\nNommez la clé "fleetupdate-agent" et copiez le token généré.',
        tip: 'Le token d’API Bearer ne sera affiché qu’une seule fois par TrueNAS.'
      },
      {
        title: '2. Configurer le périmètre de mise à jour',
        description:
          'Dans FleetUpdate-Hub, choisissez ce que vous souhaitez superviser et mettre à jour :\n• Tout (Système OS + Applications) [Recommandé]\n• Applications uniquement (Docker Compose / Charts Helm)\n• Système TrueNAS OS uniquement',
        tip: 'Vous pouvez également cibler une seule application spécifique (ex: nextcloud) en renseignant le champ optionnel targetAppName.'
      },
      {
        title: '3. Activer la protection par Snapshot ZFS (Optionnel mais recommandé)',
        description:
          'FleetUpdate-Hub créera automatiquement un snapshot ZFS de sécurité avant toute opération (par défaut sur "boot-pool" ou votre dataset applicatif "tank/ix-applications"). En cas d’échec, le système peut revenir instantanément à cet état.',
        tip: 'En cas d’échec ou de timeout lors du healthcheck, le snapshot peut être restauré automatiquement ou en 1 clic.'
      }
    ],
    formFieldsGuide: [
      { field: "Endpoint URL / IP", example: "https://192.168.1.150 ou https://truenas.local", note: "Adresse IP ou URL WebGUI de votre serveur TrueNAS." },
      { field: "Clé API TrueNAS", example: "1-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", note: "Clé API générée à l'étape 1." },
      { field: "Périmètre de mise à jour", example: "Tout (Système OS + Applications)", note: "Sélectionnez le mode de mise à jour." },
      { field: "Nom d'application cible", example: "nextcloud (ou laisser vide)", note: "Optionnel : cibler une seule application spécifique." },
      { field: "Dataset Snapshot ZFS", example: "boot-pool", note: "Nom du dataset ZFS pour la sauvegarde de sécurité." }
    ]
  }
};
...
'`,
  ...
  TRUENAS: {
    ...
    badge: 'REST API v2.0',
    summary:
      'Centralized monitoring and updates for TrueNAS (SCALE / CORE), its applications (Docker Compose apps on SCALE 24.10+ & Helm charts), backed by automated ZFS safety snapshots.',
    steps: [
      {
        title: '1. Generate an API Key in TrueNAS WebGUI',
        description:
          'Log in to your TrueNAS SCALE WebGUI as admin or root.\nIn the top right corner, click your User Profile or Gear Icon (Settings) > API Keys > Add.\nName the key "fleetupdate-agent" and copy the generated token.',
        tip: 'The Bearer API token is displayed only once by TrueNAS.'
      },
      {
        title: '2. Configure Update Scope',
        description:
          'In FleetUpdate-Hub, choose what you want to manage on this TrueNAS host:\n• All (System OS + Applications) [Recommended]\n• Applications Only (Docker Compose / Helm Charts)\n• TrueNAS OS System Only',
        tip: 'You can also target a single application (e.g. nextcloud) using the optional targetAppName field.'
      },
      {
        title: '3. Enable ZFS Snapshot Safety Checkpoint',
        description:
          'FleetUpdate-Hub automatically creates a point-in-time ZFS safety snapshot before running updates (defaults to "boot-pool" or application pool dataset "tank/ix-applications"). In case of healthcheck failure, you can roll back instantly.',
        tip: 'Rollbacks can be triggered automatically or in 1 click from the Host Details dashboard.'
      }
    ],
    formFieldsGuide: [
      { field: "Endpoint URL / IP", example: "https://192.168.1.150 or https://truenas.local", note: "IP address or WebGUI URL of your TrueNAS host." },
      { field: "TrueNAS API Key", example: "1-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx", note: "API key generated in Step 1." },
      { field: "Update Scope", example: "All (System OS + Applications)", note: "Choose update coverage." },
      { field: "Target Application Name", example: "nextcloud (or leave empty)", note: "Optional: isolate updates to a single app." },
      { field: "ZFS Snapshot Dataset", example: "boot-pool", note: "Dataset name for automated safety snapshots." }
    ]
  }
};
