    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div className="glass-panel w-full max-w-md rounded-3xl border border-rose-500/30 overflow-hidden shadow-2xl p-6 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-bold text-white">Confirmer le Rollback Manuel</h3>
            <p className="text-xs text-slate-400">Restauration à un état de sauvegarde antérieur</p>
          </div>
        </div>

        <div className="p-4 rounded-2xl bg-rose-950/20 border border-rose-900/40 text-xs text-rose-200 space-y-2">
          <div className="flex items-center gap-2 font-semibold text-rose-400">
            <AlertTriangle className="w-4 h-4" /> Attention : Opération d'Impact
          </div>
          <p className="leading-relaxed">
            Vous vous apprêtez à restaurer l'hôte <span className="font-bold text-white">{host.name}</span> à partir du snapshot / backup :
          </p>
          ...
          <p className="text-[11px] text-slate-400">
            Créé le : {new Date(backup.createdAt).toLocaleString()}
          </p>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          >
            ...
            <span>{loading ? 'Restauration en cours...' : 'Exécuter le Rollback'}</span>
          </button>
        </div>
      </div>
    </div>
