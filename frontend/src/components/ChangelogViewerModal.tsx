...
export const ChangelogViewerModal: React.FC<ChangelogViewerModalProps> = ({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-2xl rounded-3xl border border-slate-700/80 bg-[#0c1424] overflow-hidden shadow-2xl flex flex-col max-h-[85vh]">
        {/* Header */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">{t('changelog.modalTitle')}</h3>
              <p className="text-xs text-slate-400">
                {t('updates.host')}: <span className="text-slate-200 font-semibold">{host.name}</span> ({host.adapterType})
              </p>
            </div>
          </div>
          <button
              ...
              >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {item.isSecurityFix && (
                      <Badge variant="danger" size="sm">
                        <ShieldCheck className="w-3 h-3" /> {t('changelog.securityPatch')}
                      </Badge>
                    )}
                  </div>
                  {item.releaseDate && (
                ...
                {item.detailsUrl && (
                  <div className="pt-2">
                    ...
                    >
                      <span>{t('changelog.officialSource')}</span>
