...
export const EditHostModal: React.FC<EditHostModalProps> = ({
  ...
  const handleUpdate = async (formData: any) => {
    ...
    try {
      ...
      addToast("success", t('common.success'), res.message || t('hosts.hostUpdatedSuccess'));
      onSuccess(res.host);
      onClose();
    } catch (err: any) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/75 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-2xl rounded-3xl border border-cyan-500/30 overflow-hidden shadow-2xl flex flex-col max-h-[90vh] bg-[#0c1424] text-slate-100 shadow-cyan-500/10">
        {/* Header */}
        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/60">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-lg font-bold text-white">{t('hosts.editHostTitle')}</h3>
              <p className="text-xs text-slate-400">
                {t('hosts.editHostSubtitle')}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          {/* Adapter Selection */}
          <div>
            <label className="text-xs font-semibold text-slate-300 block mb-2">{t('hosts.selectModule')}</label>
          ...
          {selectedAdapter ? (
            ...
            <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 text-xs flex items-center gap-2">
              <AlertCircle className="w-4 h-4" /> {t('hosts.emptyList')}
            </div>
          )}
        </div>
      </div>
