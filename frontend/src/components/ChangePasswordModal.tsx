...
export const ChangePasswordModal: React.FC<ChangePasswordModalProps> = ({ onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      addToast("error", t('common.error'), t('auth.passwordsDoNotMatch'));
      return;
    }
    if (newPassword.length < 8) {
      addToast("error", t('common.error'), t('auth.passwordMinLength'));
      return;
    }
    ...
    try {
      ...
      addToast("success", t('common.success'), res.message || t('auth.passwordChangedSuccess'));
      onClose();
    } catch (err: any) {
      addToast("error", t('common.error'), err.response?.data?.message || err.message || "Failed to update password.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-[#0b1120] border border-slate-800 rounded-2xl w-full max-w-md p-6 shadow-2xl space-y-6 relative">
        <div className="flex items-center justify-between border-b border-slate-800/80 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-slate-100 text-base">{t('nav.changePassword')}</h3>
