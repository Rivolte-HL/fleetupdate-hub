...
export const DynamicAdapterForm: React.FC<DynamicAdapterFormProps> = ({
  ...
  const renderField = (field: FormFieldDefinition, isCred: boolean) => {
    ...
    const placeholder = isCred && isEdit
      ? (t('hosts.leaveEmptySecret') || 'Leave empty to preserve existing secret')
      : field.placeholder;
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-4">
          <h4 className="text-xs font-bold uppercase tracking-wider text-cyan-400 flex items-center gap-2">
            <Server className="w-4 h-4" /> {t('hosts.hostGeneralSettings')}
          </h4>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-300">{t('hosts.hostNameLabel')} *</label>
              <input
                type="text"
                required
                placeholder="ex: pve-cluster-01"
                value={name}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-semibold text-slate-300">{t('hosts.endpointUrlLabel')} *</label>
            <input
              type="text"
              required
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-slate-300">{t('hosts.descriptionLabel')}</label>
          <input
            type="text"
            placeholder="Production virtualization server..."
            value={description}
      ...
      {adapter.connectionFields.length > 0 && (
        <div className="space-y-4 pt-4 border-t border-slate-800">
          <h4 className="text-xs font-bold uppercase tracking-wider text-amber-400 flex items-center gap-2">
            <Server className="w-4 h-4" /> {adapter.displayName}
          </h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400 flex items-center gap-2">
                <Key className="w-4 h-4" /> {t('hosts.credentialsVaultSection')}
              </h4>
              {isEdit && (
                <p className="text-[11px] text-slate-400 mt-0.5">
                  {t('hosts.leaveEmptySecret')}
                </p>
              )}
            </div>
            <span className="text-[11px] text-slate-400 flex items-center gap-1">
              <Shield className="w-3.5 h-3.5 text-emerald-400" /> AES-256-GCM
            </span>
          </div>
      ...
      <div className="pt-4 border-t border-slate-800 flex items-center justify-end gap-3">
        ...
        >
          {loading ? t('common.loading') : (isEdit ? t('common.save') : t('hosts.addHostButton'))}
        </button>
      </div>
    </form>
