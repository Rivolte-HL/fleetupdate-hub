...
export const HostDetailPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Applications & Services (TrueNAS Apps / Managed Services) */}
      {(host.adapterType === "TRUENAS" || (host.metadata?.apps && host.metadata.apps.length > 0)) && (
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-300">
                  Applications & Services ({host.metadata?.apps?.length || 0})
                </h3>
                <p className="text-[11px] text-slate-500">
                  Découverte automatique des applications TrueNAS (Docker Compose & Charts Helm)
                </p>
              </div>
            </div>
            <span className="text-xs font-mono font-bold text-sky-400 bg-sky-500/10 border border-sky-500/20 px-2.5 py-1 rounded-xl">
              {host.metadata?.appsWithUpdateCount ? `${host.metadata.appsWithUpdateCount} mise(s) à jour disponible(s)` : 'Toutes à jour'}
            </span>
          </div>

          {!host.metadata?.apps || host.metadata.apps.length === 0 ? (
            <div className="p-8 text-center text-xs text-slate-500 rounded-2xl bg-slate-900/40 border border-slate-800/80">
              Aucune application listée pour le moment. Cliquez sur le bouton "Rafraîchir" en haut pour interroger l'API TrueNAS.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {host.metadata.apps.map((app: any) => (
                <div
                  key={app.id || app.name}
                  className={`p-4 rounded-2xl border transition-all ${
                    app.hasUpdate
                      ? "bg-amber-500/5 border-amber-500/30 hover:border-amber-500/50 shadow-sm shadow-amber-500/5"
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2.5 min-w-0">
                      {app.icon ? (
                        <img src={app.icon} alt={app.name} className="w-8 h-8 rounded-xl object-contain shrink-0 bg-slate-800/80 p-1 border border-slate-700/60" />
                      ) : (
                        <div className="p-2 rounded-xl bg-slate-800 text-sky-400 shrink-0 border border-slate-700/60">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="text-[10px] font-mono text-slate-500">
                          {app.type === 'DOCKER_APP' ? 'Docker Compose App' : 'Helm Chart Release'}
                        </span>
                      </div>
                    </div>
                    <Badge variant={app.status === 'RUNNING' || app.status === 'ACTIVE' ? 'success' : 'neutral'} size="sm">
                      {app.status}
                    </Badge>
                  </div>

                  <div className="mt-3 pt-3 border-t border-slate-800/60 flex items-center justify-between text-xs font-mono">
                    <span className="text-slate-400">Actuel: <strong className="text-slate-200">{app.currentVersion}</strong></span>
                    {app.hasUpdate ? (
                      <span className="text-amber-400 flex items-center gap-1 font-bold">
                        <ArrowUpCircle className="w-3.5 h-3.5" /> {app.targetVersion}
                      </span>
                    ) : (
                      <span className="text-emerald-400/90 flex items-center gap-1 text-[11px] font-semibold">
