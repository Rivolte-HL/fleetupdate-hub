...
import { Server, ShieldCheck, AlertTriangle, Activity, RefreshCw, Plus, History, Search, ChevronRight, Zap } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
  ...
  const handleRefreshAll = async () => {
    ...
    try {
      ...
      setHosts(res.hosts);
      addToast('success', t('common.success'), res.message || `${res.summary?.updatesFound || 0} ${t('common.updatesAvailable')}`);
    } catch (err: any) {
      addToast('error', t('common.error'), err.response?.data?.message || err.message);
    } finally {
    ...
    <div className="space-y-7">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            <h2 className="text-2xl font-extrabold text-white tracking-tight">{t('dashboard.title')}</h2>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          {hostsWithUpdates > 0 && (
            ...
            >
              ...
              <span>{t('bulk.updateAllButton') || 'Update All'} ({hostsWithUpdates})</span>
            </button>
          )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          subtitle={`${totalHosts - onlineHosts} ${t('common.offline')}`}
          icon={Server}
          variant="cyan"
        />
        <StatCard
          ...
          subtitle={`${hostsWithUpdates} ${t('common.updatesAvailable')}`}
          icon={Activity}
        ...
        <StatCard
          ...
          subtitle={t('dashboard.kpiRebootsSubtitle')}
          icon={AlertTriangle}
          variant={rebootsRequired > 0 ? 'rose' : 'slate'}
        />
        <StatCard
          title={t('dashboard.kpiSecurity')}
          value="100%"
          subtitle={t('dashboard.kpiSecuritySubtitle')}
          icon={ShieldCheck}
          variant="emerald"
        />
      </div>
      ...
      <div className="space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <h3 className="text-base font-bold text-white">{t('nav.hosts')}</h3>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {[
              ...
              { id: 'UPDATES', label: `${t('dashboard.filterUpdates')} (${hostsWithUpdates})` },
              { id: 'ONLINE', label: `${t('dashboard.filterOnline')} (${onlineHosts})` },
              { id: 'OFFLINE', label: `${t('dashboard.filterOffline')} (${totalHosts - onlineHosts})` },
              { id: 'REBOOT', label: `${t('dashboard.filterReboot')} (${rebootsRequired})` }
            ].map((f) => (
              <button
                key={f.id}
                type="button"
        ...
        {loading ? (
          ...
          <div className="glass-panel p-10 rounded-2xl text-center space-y-3">
            ...
            <div>
              <h4 className="text-sm font-bold text-white">{t('dashboard.noHostsFound')}</h4>
              <p className="text-xs text-slate-400 mt-0.5">
                {hosts.length === 0 ? t('hosts.addNewHostSubtitle') : t('dashboard.noHostsMatchSearch')}
              </p>
            </div>
            {hosts.length === 0 && (
              <button
      ...
      <div className="glass-panel p-5 rounded-3xl space-y-3.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-white">{t('dashboard.recentTasks')}</h3>
          </div>
          <button
            type="button"
          ...
          >
            {t('updates.tasksHistory')} <ChevronRight className="w-3.5 h-3.5" />
          </button>
        </div>

        {tasks.length === 0 ? (
          <p className="text-xs text-slate-500 py-3 text-center">{t('dashboard.noRecentTasks')}</p>
        ) : (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-bold text-xs text-white group-hover:text-cyan-300 transition-colors truncate">
                    {tItem.host?.name || t('updates.host')}
                  </span>
                  <Badge
                    variant={
                      tItem.status === 'SUCCESS'
                        ? 'success'
                        : tItem.status === 'ROLLED_BACK'
                        ? 'warning'
                        : tItem.status === 'FAILED'
                        ? 'danger'
                        : 'brand'
                    ...
                    size="sm"
                    pulse={!['SUCCESS', 'FAILED', 'ROLLED_BACK'].includes(tItem.status)}
                  >
                    {tItem.status}
                  </Badge>
                </div>
                ...
                <div className="flex items-center justify-between text-[11px] text-slate-400">
                  <span className="font-mono text-cyan-400 truncate">{t('updates.step')}: {tItem.currentStep}</span>
                  <span>{new Date(tItem.startedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
              </div>
            ))}
          </div>
