...
if [ -n "$DATABASE_URL_FILE" ] && [ -f "$DATABASE_URL_FILE" ]; then
  ...
  export DATABASE_URL="postgresql://${POSTGRES_USER:-fleet_user}:${POSTGRES_PASSWORD:-fleet_password_change_me}@db:5432/${POSTGRES_DB:-fleetupdate}?schema=public"
fi
...
if [ -n "$MASTER_ENCRYPTION_KEY_FILE" ] && [ -f "$MASTER_ENCRYPTION_KEY_FILE" ]; then
  ...
  if [ -f "$PERSISTENT_KEY_FILE" ]; then
    ...
    chmod 600 "$PERSISTENT_KEY_FILE" 2>/dev/null || true
    echo "======================================================================"
    echo "🔐 [Zero-Trust] Automatically generated AES-256 Master Encryption Key."
    echo "Saved to persistent file: $PERSISTENT_KEY_FILE (permissions: 0600)"
    echo "Note: The key is stored securely on disk and is never exposed in container logs."
    echo "======================================================================"
  fi
fi
...
if [ -n "$JWT_SECRET_FILE" ] && [ -f "$JWT_SECRET_FILE" ]; then
  ...
  if [ -f "$PERSISTENT_JWT_FILE" ]; then
    ...
    chmod 600 "$PERSISTENT_JWT_FILE" 2>/dev/null || true
  fi
fi
