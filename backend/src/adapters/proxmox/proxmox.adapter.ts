...
export class ProxmoxAdapter extends BaseServiceAdapter {
  ...
  public async rollback(
    ...
    onProgress?.('ROLLBACK', `Inspecting Proxmox VE restore point (${backupIdentifier})...`);
    const logs = [
      `Pre-update restore point: ${backupIdentifier}`,
      'Automated in-place hypervisor kernel rollback is restricted to prevent split-brain cluster states.',
      'Guest VMs and vzdump archives remain intact on configured storage.'
    ];
    return {
      success: false,
      restoredVersion: host.currentVersion || 'Unmodified',
      logs,
      message: `Automatic Proxmox VE hypervisor rollback is restricted to prevent cluster inconsistency. Please restore using vzdump checkpoint "${backupIdentifier}" or reboot to the previous kernel via GRUB.`
    };
  }
}
