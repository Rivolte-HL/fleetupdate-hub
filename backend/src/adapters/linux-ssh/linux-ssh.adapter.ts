...
export class LinuxSshAdapter extends BaseServiceAdapter {
  ...
  private getClient(host: Host, credentials: TargetCredentials): { client: SshClient; pkgMgr: string } {
    ...
    const expectedHostFingerprint = meta.expectedHostFingerprint || credentials.hostFingerprint || credentials.keyFingerprint;
  ...
  private async parseUpgradablePackages(client: SshClient, pkgMgr: string): Promise<LinuxPackage[]> {
    ...
    try {
      if (pkgMgr === 'apt') {
        ...
        await client.executeCommand('DEBIAN_FRONTEND=noninteractive sudo apt-get update -qq 2>/dev/null || sudo apt-get update -qq 2>/dev/null || true');
  ...
  public async createBackup(host: Host, credentials: TargetCredentials, backupName?: string): Promise<BackupResult> {
    ...
    const meta = (host.metadata as any) || {};
    const username = (meta.username || credentials.username || 'fleetupdate').trim().toLowerCase();
    const isRoot = username === 'root';
    const sudo = isRoot ? '' : 'sudo ';

    // Create /etc backup archive in a secure root-only directory
    try {
      const backupDir = '/var/backups/fleetupdate';
      await client.executeCommand(`${sudo}mkdir -p -m 700 ${backupDir}`);
      await client.executeCommand(
        `${sudo}tar --warning=no-file-changed -czf ${backupDir}/${snapId}_etc.tar.gz /etc 2>/dev/null || ${sudo}tar -czf ${backupDir}/${snapId}_etc.tar.gz /etc 2>/dev/null || true`
      );
      await client.executeCommand(`${sudo}chmod 600 ${backupDir}/${snapId}_etc.tar.gz 2>/dev/null || true`);

      const checkRes = await client.executeCommand(`[ -s ${backupDir}/${snapId}_etc.tar.gz ] && echo "EXISTS" || echo "MISSING"`);
      if (checkRes.stdout.trim() === 'EXISTS') {
        return {
          success: true,
          backupId: snapId,
          backupType: 'ETC_ARCHIVE',
          message: 'System configuration archive (/etc) saved securely in /var/backups/fleetupdate.'
        };
      }
      ...
      return {
        ...
        message: 'Pre-update safety checkpoint recorded (without file archive).'
      };
    } catch (e: any) {
  ...
  public async applyUpdate(
    ...
    if (pkgMgr === 'apt') {
      onProgress?.('UPDATING', 'Refreshing APT repositories (apt-get update)...');
      const updateRes = await client.executeCommand(`DEBIAN_FRONTEND=noninteractive ${sudo}apt-get -o DPkg::Lock::Timeout=60 update`);
      ...
      const upgradeRes = await client.executeCommand(
        `DEBIAN_FRONTEND=noninteractive NEEDRESTART_MODE=a ${sudo}apt-get -y -q -o Dpkg::Use-Pty=0 -o DPkg::Lock::Timeout=120 -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" dist-upgrade`
      );
      ...
      const cleanRes = await client.executeCommand(
        `DEBIAN_FRONTEND=noninteractive ${sudo}apt-get -y -o DPkg::Lock::Timeout=60 autoremove && ${sudo}apt-get autoclean`
      );
  ...
  public async rollback(
    ...
    onProgress?.('ROLLBACK', `Checking configuration backup for ${safeBackupId}...`);
    try {
      const primaryPath = `/var/backups/fleetupdate/${safeBackupId}_etc.tar.gz`;

      const checkPrimary = await client.executeCommand(`[ -s "${primaryPath}" ] && echo "PRIMARY" || echo "MISSING"`);
      let archivePath = '';

      if (checkPrimary.stdout.trim() === 'PRIMARY') {
        archivePath = primaryPath;
      }

      if (!archivePath) {
        onProgress?.('ROLLBACK', `No /etc archive found in /var/backups/fleetupdate for ${safeBackupId}; checkpoint acknowledged.`);
        return {
          success: true,
          restoredVersion: host.currentVersion || 'Current',
          logs: [`No archive file present on host in /var/backups/fleetupdate for ${safeBackupId}. System files were not modified.`],
          message: 'Rollback acknowledged. No configuration files needed restoring.'
        };
      }

      onProgress?.('ROLLBACK', `Restoring /etc configuration from ${archivePath}...`);
      const res = await client.executeCommand(`${sudo}tar --warning=no-file-changed --no-same-owner -xzf "${archivePath}" -C /`);
      if (res.code !== 0) {
        return {
          success: false,
          restoredVersion: 'Restore failed',
          logs: [`Error extracting archive ${safeBackupId} from ${archivePath}: ${res.stderr || res.stdout}`],
          message: `Linux rollback failed (Code ${res.code}): ${res.stderr || res.stdout || 'Archive extraction failed'}`
        };
      }
