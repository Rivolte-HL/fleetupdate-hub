      ...
      try {
        conn.connect({
          ...
          hostVerifier: (hashedKey: string) => {
            if (!this.config.expectedHostFingerprint) {
              return true;
            }
            const cleanExpected = this.config.expectedHostFingerprint.replace(/^SHA256:/i, '').trim().toLowerCase();
            const cleanReceived = String(hashedKey).replace(/^SHA256:/i, '').trim().toLowerCase();
            const isMatch = cleanExpected === cleanReceived;
            if (!isMatch) {
              console.error(`[SSH Security Error] Host key fingerprint mismatch for ${this.config.host}! Expected: ${cleanExpected}, Received: ${cleanReceived}`);
            }
            return isMatch;
          }
        });
