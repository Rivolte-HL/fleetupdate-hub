...
export class DockerAdapter extends BaseServiceAdapter {
  ...
  private async updateContainerWithRecreation(
    ...
    onProgress?.(overrideImage ? 'ROLLBACK' : 'UPDATING', `[${containerName}] Inspecting container configuration & mounts...`);
    const inspect = await client.inspectContainer(containerName);
    const targetImage = overrideImage || inspect.Config?.Image || containerName;
    const isRunning = inspect.State?.Running ?? true;

    if (!overrideImage) {
      onProgress?.('UPDATING', `[${containerName}] Pulling latest image layer for ${targetImage}...`);
      await client.pullImage(targetImage);
    } else {
      onProgress?.('ROLLBACK', `[${containerName}] Restoring from preserved rollback image: ${overrideImage}...`);
    }

    const newImgInspect = await client.inspectImage(targetImage);

    // If image ID is unchanged, container is running and not rolling back, notify
    if (!overrideImage && newImgInspect.Id === inspect.Image) {
      onProgress?.('UPDATING', `[${containerName}] Image ${targetImage} is already identical to local layer.`);
      return { success: true, log: `Container ${containerName} is already running the latest image layer.` };
    }

    onProgress?.(
      overrideImage ? 'ROLLBACK' : 'UPDATING',
      `[${containerName}] Applying target image layer (${newImgInspect.Id.slice(7, 19)}). Stopping container gracefully (timeout: ${stopTimeout}s)...`
    );
    if (isRunning) {
    ...
    return {
      success: true,
      log: `Container ${containerName} successfully ${overrideImage ? 'restored to' : 'upgraded to'} image ${targetImage} (${newImgInspect.Id.slice(7, 19)}).`
    };
  }
