...
export class DockerClient {
  ...
  private async request<T = any>(
    ...
    // Strict TLS certificate validation by default (Zero-Trust Policy)
    let rejectUnauthorized = true;
    if (this.config.allowSelfSigned === true) {
      rejectUnauthorized = false;
      console.warn(`[SECURITY WARNING] TLS certificate verification is disabled (allowSelfSigned: true) for Docker host: ${this.host}:${this.port}`);
  ...
  public async connectNetwork(networkId: string, containerId: string, endpointConfig?: any): Promise<any> {
    const safeNetId = encodeURIComponent(networkId.trim());
    const body: any = { Container: containerId.trim() };
    if (endpointConfig) {
      body.EndpointConfig = endpointConfig;
    }
    return this.request(`/networks/${safeNetId}/connect`, 'POST', body);
  }

  public async disconnectNetwork(networkId: string, containerId: string, force: boolean = false): Promise<any> {
    const safeNetId = encodeURIComponent(networkId.trim());
    return this.request(`/networks/${safeNetId}/disconnect`, 'POST', {
      Container: containerId.trim(),
      Force: force
    });
  }

  public async pruneImages(danglingOnly: boolean = true): Promise<any> {
    const filters = JSON.stringify({ dangling: [danglingOnly ? 'true' : 'false'] });
    return this.request(`/images/prune?filters=${encodeURIComponent(filters)}`, 'POST');
  }

  public async getContainerLogs(id: string, tail: number = 30): Promise<string> {
    const safeId = encodeURIComponent(id.trim());
    return this.request(`/containers/${safeId}/logs?stdout=1&stderr=1&tail=${tail}`, 'GET');
  }
}
