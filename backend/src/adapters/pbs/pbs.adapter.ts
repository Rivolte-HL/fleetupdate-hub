...
export class ProxmoxBackupServerAdapter extends BaseServiceAdapter {
  ...
  public async rollback(
    ...
    onProgress?.('ROLLBACK', `Inspecting PBS restore point (${backupIdentifier})...`);
    return {
      success: false,
      restoredVersion: host.currentVersion || 'Unmodified',
      logs: [
        `PBS safety checkpoint: ${backupIdentifier}`,
        'Automated package downgrade on Proxmox Backup Server requires manual administrator validation to protect datastore integrity.'
      ],
