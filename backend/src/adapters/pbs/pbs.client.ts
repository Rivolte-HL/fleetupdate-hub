...
export class PbsClient {
  ...
  public async request<T = any>(endpoint: string, method: "GET" | "POST" | "DELETE" = "GET", body?: any): Promise<T> {
    ...
    if (isHttps && isSelfSignedAllowed) {
      console.warn(`[SECURITY WARNING] TLS certificate verification is disabled (allowSelfSigned: true) for PBS host: ${parsed.hostname}`);
    }
  ...
  public async getTasks(node: string = "localhost"): Promise<any[]> {
    const safeNode = encodeURIComponent(node.trim());
    try {
      const res = await this.request(`/nodes/${safeNode}/tasks`, "GET");
      return Array.isArray(res) ? res : [];
    } catch {
      return [];
    }
