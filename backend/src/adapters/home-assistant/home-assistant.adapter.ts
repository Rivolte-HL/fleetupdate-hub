...
export class HomeAssistantAdapter extends BaseServiceAdapter {
  ...
  public async rollback(
    ...
    onProgress?.('ROLLBACK', `Inspecting Home Assistant supervisor checkpoint (${backupIdentifier})...`);
    return {
      success: false,
      restoredVersion: host.currentVersion || 'Unmodified',
      logs: [
        `Home Assistant supervisor checkpoint: ${backupIdentifier}`,
        'Automatic supervisor restore requires manual user confirmation in Home Assistant (Settings -> System -> Backups).'
      ],
