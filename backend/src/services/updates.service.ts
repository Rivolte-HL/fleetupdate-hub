...
export class UpdatesService {
  ...
  public static async triggerUpdate(options: TriggerUpdateOptions) {
    ...
    const result = await prisma.$transaction(async (tx) => {
      // Verrouillage transactionnel pessimiste (FOR UPDATE) pour éliminer les race conditions
      const hosts: any[] = await tx.$queryRaw`
        SELECT id, name, "currentVersion", "targetVersion"
        FROM hosts
        WHERE id = ${hostId}
        FOR UPDATE
      `;
