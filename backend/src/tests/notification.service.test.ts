...
describe('NotificationService Multi-Channel & Actions Tests', () => {
  ...
  it('should block SSRF attempts to cloud metadata and invalid protocols in validateOutboundUrl', () => {
    // Cloud metadata block
    assert.throws(() => {
      NotificationService.validateOutboundUrl('http://169.254.169.254/latest/meta-data');
    }, /SSRF Protection/);

    assert.throws(() => {
      NotificationService.validateOutboundUrl('http://metadata.google.internal/computeMetadata/v1');
    }, /SSRF Protection/);

    // Invalid protocol block
    assert.throws(() => {
      NotificationService.validateOutboundUrl('ftp://example.com/file');
    }, /Forbidden protocol/);

    // Valid URL should pass
    assert.doesNotThrow(() => {
      NotificationService.validateOutboundUrl('https://hooks.slack.com/services/xxx');
    });
  });
