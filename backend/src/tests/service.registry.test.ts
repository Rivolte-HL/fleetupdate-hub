...
test('ServiceRegistry Dynamic Module Registry Tests', async (t) => {
  ...
  await t.test('should list all registered adapter metadata', () => {
    ...
    assert.ok(types.includes(HostType.TRUENAS));
  });

  await t.test('should retrieve TrueNAS adapter correctly', () => {
    const adapter = registry.getAdapter(HostType.TRUENAS);
    assert.ok(adapter);
    const meta = adapter.getMetadata();
    assert.equal(meta.type, HostType.TRUENAS);
    assert.ok(meta.credentialFields.some(f => f.name === 'apiKey'));
    assert.ok(meta.connectionFields.some(f => f.name === 'targetScope'));
  });
