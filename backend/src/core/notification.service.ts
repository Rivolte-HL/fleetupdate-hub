...
export class NotificationService {
  ...
  /**
   * Validates that an outbound webhook / notification URL is safe from SSRF.
   * Blocks non-HTTP(S) protocols, cloud metadata services (169.254.169.254),
   * and loopback in production unless explicitly allowed in development.
   */
  public static validateOutboundUrl(rawUrl: string): void {
    ...
    // 1. Block Cloud Metadata Services (AWS, GCP, Azure, DigitalOcean, Oracle)
    if (host === '169.254.169.254' || host === 'metadata.google.internal' || host === 'metadata') {
      throw new Error('Access to cloud metadata services is strictly forbidden (SSRF Protection).');
    }

    // 2. Block Loopback / Localhost in production mode
    if (config.env === 'production') {
      if (host === 'localhost' || host === '127.0.0.1' || host === '::1' || host === '0.0.0.0' || host.startsWith('127.')) {
        throw new Error('Loopback addresses (localhost / 127.0.0.1) are forbidden in production mode.');
      }
    }
  }
  ...
  public static async sendHomeAssistantNotification(
    ...
    try {
      ...
      this.validateOutboundUrl(endpoint);
  ...
  public static async sendNextcloudTalkNotification(
    ...
    try {
      ...
      this.validateOutboundUrl(targetUrl);
  ...
  public static async sendDiscordWebhook(
    ...
    try {
      this.validateOutboundUrl(webhookUrl);
  ...
  public static async sendGenericWebhook(
    ...
    try {
      this.validateOutboundUrl(genericConfig.webhookUrl);
