...
async function initDatabaseDefaults(): Promise<void> {
  try {
    ...
    if (count === 0) {
      ...
      rootLogger.info(`Initial admin password: ${adminPassword}`);
      if (!isCustomPassword) {
        rootLogger.info('🔒 A cryptographically secure random password was generated automatically');
      }
      rootLogger.warn('⚠️ Please change this password upon first login and enable 2FA TOTP!');
    }
  } catch (err) {
